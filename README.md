...
2024 DATA·AI 분석 경진대회에 참가하며 진행한 팀 프로젝트의 실험 기록을, 참여자
본인이 학습·참고 목적으로 정리해 공개한 것이다. 별도의 라이선스를 두지 않았으므로
코드 재사용이 필요하면 문의 바란다. 데이터셋은 대회 제공분이라 포함하지 않았다.
